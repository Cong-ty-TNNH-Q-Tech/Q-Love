...
func TestCourtWorker_EvaluateExpiredCases_VoteCountError(t *testing.T) {
	mockCourt := new(mockCourtRepo)
	mockViolation := new(mockUserViolationRepoCourt)
	mockWallet := new(mockWalletRepoCourt)
	
	worker := NewCourtWorker(mockCourt, mockViolation, nil, zap.NewNop(), mockWallet, &mockTxManagerCourt{})
...
func TestCourtWorker_EvaluateExpiredCases_VoteCountError(t *testing.T) {
	...
	caseID := uuid.New()
	cases := []models.CourtCase{{ID: caseID}}
	
	mockCourt.On("GetExpiredVotingCases", mock.Anything).Return(cases, nil)
	mockCourt.On("CountVotesByCase", mock.Anything, caseID).Return(int64(0), int64(0), errors.New("count err"))
...
func TestCourtWorker_EvaluateExpiredCases_GetCasesError(t *testing.T) {
	mockCourt := new(mockCourtRepo)
	mockViolation := new(mockUserViolationRepoCourt)
	mockWallet := new(mockWalletRepoCourt)
	...
	mockCourt.On("GetExpiredVotingCases", mock.Anything).Return([]models.CourtCase{}, errors.New("db error"))

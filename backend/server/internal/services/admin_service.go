...
func NewAdminService(violationRepo repository.UserViolationRepository, courtCaseRepo repository.CourtCaseRepository, r2Client *storage.R2Client) AdminService {
	return &adminService{
...
func (s *adminService) OverrideCourtCase(ctx context.Context, caseID uuid.UUID, status string) error {
	return s.courtCaseRepo.UpdateStatus(ctx, caseID, status)
}
